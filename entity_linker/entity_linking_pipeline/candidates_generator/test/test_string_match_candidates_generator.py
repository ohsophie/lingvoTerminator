...
from utils.paths import ENTITY_LINKER_PATH
...
class TestStringMatch(unittest.TestCase):
    ...
    def setUp(self):
        config = load_config()
        config['entity_linker']['path_to_json_file'] = os.path.join(ENTITY_LINKER_PATH, 'test_data/test_dump.json')
        ...
        self._candidates_generator = StringMatchCandidatesGenerator()
